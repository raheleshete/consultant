jQuery(document).ready(function($) {
	...
	*/
        ...
        if (geojson.length == 0 || geojson == '0') {
            ...
            if (map.id == 'map-canvas') {
			   ...
			   //console.log(map_type);
                ...
                gpsPlotterMap.addControl(new L.Control.Layers({
    ...
    function createMarker(latitude, longitude, session_id, speed, direction, distance, locationMethod, gpsTime,
        ...
        if (finalLocation) {
            gpsplotterMarker = L.marker(new L.LatLng(latitude, longitude), {title: title, icon: markerIcon, zIndexOffset: 999}).bindPopup(popupWindowText).addTo(map);
        } else {
            gpsplotterMarker = L.marker(new L.LatLng(latitude, longitude), {title: title, icon: markerIcon}).bindPopup(popupWindowText).addTo(map);
        }
