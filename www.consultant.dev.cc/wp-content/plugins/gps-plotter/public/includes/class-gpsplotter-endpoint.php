<?php
/**
 * Handles GET requests from phone by setting up and endpoint /gpsplotter/
...
/**
 * Gps_Plotter_Endpoint Class
 *
 * @since 1.0.0
 */
class Gps_Plotter_Endpoint {
        
	/**
	 * Set up the Gps Plotter Endpoint Class
	 *
	 * @since 1.0.0
	 */        
    public function __construct() {            
	    add_action( 'init',              array( $this, 'add_gpsplotter_endpoint' ) );
		add_action( 'template_redirect', array( $this, 'process_gpsplotter_query' ), -1 );
		add_filter( 'query_vars',        array( $this, 'gpsplotter_query_vars' ) );
    }
	 ...
	 * @since 1.0.0
	 */
    public function add_gpsplotter_endpoint( $rewrite_rules ) {
        add_rewrite_endpoint( 'gpsplotter', EP_ROOT );
    }
	 ...
	 * @return void
	 */ 
    public function process_gpsplotter_query() {
        global $wp_query;

        if ( ! isset($wp_query->query_vars['gpsplotter'] ) ) {
            return;
        }
        
        switch ( $wp_query->query_vars['gpsplotter'] ) {
            case 'nonce':
	 ...
	 */
    public function gpsplotter_query_vars( $query_vars ) {
        $query_vars[] = 'latitude';
