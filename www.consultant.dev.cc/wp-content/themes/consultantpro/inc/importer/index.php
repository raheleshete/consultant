...
/**
 * consultant_Setup class
 */
if (!class_exists('consultant_Setup')) {
	class consultant_Setup {
		...
		public function __construct() {
			...
			if ( apply_filters( $this->theme_name . '_enable_setup_wizard', true ) && current_user_can( 'manage_options' )  ) {
				$this->public_base_url = get_template_directory_uri().'/inc/importer';
		...
		public function admin_menus() {
			add_theme_page(esc_html__('Import Demo Data','consultantpro'), esc_html__('Import Demo Data','consultantpro'), 'manage_options', $this->theme_name.'-setup', array($this,'setup_wizard') );
		}
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text' => esc_html__('...verifying','consultantpro'),
			) );
			...
			$this->steps = array(
				'introduction' => array(
					'name'    =>  esc_html__( 'Introduction', 'consultantpro' ),
		...
		public function setup_wizard_footer() {
			?>
				...
				<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'consultantpro' ); ?></a>
				<?php endif; ?>
			</body>
			<?php
	...
	public function envato_setup_introduction() {
		if(isset($_REQUEST['export'])){
			...
			?>
			<h1><?php esc_html_e( 'Current Settings:','consultantpro'); ?></h1>
			<p><?php esc_html_e( 'Widget Positions:','consultantpro'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($widget_positions);?></textarea>
			<p><?php esc_html_e( 'Widget Options:','consultantpro'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($widget_options);?></textarea>
			<p><?php esc_html_e( 'Menu IDs:','consultantpro'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($menu_ids);?></textarea>
			<p><?php esc_html_e( 'Custom Options:','consultantpro'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($my_options);?></textarea>
			<p><?php esc_html_e( 'Copy these values into your PHP code when distributing/updating the theme.','consultantpro'); ?></p>
			<?php
		}else {
			?>
			<h1><?php esc_html_e( 'Welcome to consultant install!', 'consultantpro' ); ?></h1>
			<p><?php  esc_html_e( "Everyone hates to install-but you will love this! Our rocket-fast install wizard will do most of the work for you. We'll have a couple of questions along the way, but right now you are ", 'consultantpro');?><u><?php esc_html_e('only minutes','consultantpro'); ?></u><?php esc_html_e( " away from having an awesome looking blog for the whole world to see.", 'consultantpro' ); ?></p>
			<p><?php esc_html_e( 'By the way, if you are slammed for time and can\'t spare 5 minutes right now, no problem! Come back to this "dashboard" page when you have a little more time and we\'ll be waiting right here-ready to go!', 'consultantpro' ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'consultantpro' ); ?></a>
				<a href="<?php echo esc_url( wp_get_referer() ? wp_get_referer() : admin_url( '' ) ); ?>"
				   class="button button-large"><?php esc_html_e( 'Not right now', 'consultantpro' ); ?></a>
			</p>
			<?php
		}
	}
	...
	public function envato_setup_default_plugins() {
		...
		?>
		<h1><?php esc_html_e( 'Default Plugins', 'consultantpro' ); ?></h1>
		<form method="post">
			...
			?>
				<p><?php esc_html_e( 'Some of these you Must Have and some of them you\'ll Want to Have...', 'consultantpro' ); ?></p>
				...
				echo '<p><strong>'.esc_html__('Good news! All plugins are already installed and up to date. Please continue.','consultantpro').'</strong></p>';
			} ?>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e( 'Continue', 'consultantpro' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'consultantpro' ); ?></a>
	...
	public function ajax_plugins(){
		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])){
			wp_send_json_error(array('error'=>1,'message'=>esc_html__('No Slug Found','consultantpro')));
		}
		$json = array();
		...
		foreach($plugins['activate'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Activating Plugin','consultantpro'),
				);
				break;
			}
		}
		foreach($plugins['update'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Updating Plugin','consultantpro'),
				);
				break;
			}
		}
		foreach($plugins['install'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Installing Plugin','consultantpro'),
				);
				break;
			}
		}
		...
		if($json){
			...
			wp_send_json(array('done'=>1,'message'=>esc_html__('Success','consultantpro')));
		}
		exit;
	...
	private function _content_default_get(){
		...
		$content['pages'] = array(
			'title' => esc_html__( 'Pages', 'consultantpro' ),
			'description' => esc_html__( 'Creates a new "blank" page for you to use.', 'consultantpro' ),
			'pending' => esc_html__( 'Pending.', 'consultantpro' ),
			'installing' => esc_html__( "Default--It's not your fault!", 'consultantpro' ),
			'success' => esc_html__( 'Success.', 'consultantpro' ),
			'install_callback' => '_content_install_pages',
		);
		$content['formidable'] = array(
			'title' => esc_html__( 'Install Forms', 'consultantpro' ),
			'description' => esc_html__( 'Creates all forms', 'consultantpro' ),
			'pending' => esc_html__( 'Pending.', 'consultantpro' ),
			'installing' => esc_html__( "Default--It's not your fault!", 'consultantpro' ),
			'success' => esc_html__( 'Success.', 'consultantpro' ),
			'install_callback' => '_content_install_formidable',
		);
		$content['widgets'] = array(
			'title' => esc_html__( 'Widgets', 'consultantpro' ),
			'description' => esc_html__( 'Cool "widgets" on your sidebar that you will make your page come alive with options.', 'consultantpro' ),
			'pending' => esc_html__( 'Pending.', 'consultantpro' ),
			'installing' => esc_html__( 'Installing Default Widgets.', 'consultantpro' ),
			'success' => esc_html__( 'Success.', 'consultantpro' ),
			'install_callback' => '_content_install_widgets',
		);
		$content['settings'] = array(
			'title' => esc_html__( 'Settings', 'consultantpro' ),
			'description' => esc_html__( 'General settings help you to customize your site.', 'consultantpro' ),
			'pending' => esc_html__( 'Pending.', 'consultantpro' ),
			'installing' => esc_html__( 'Installing Default Settings.', 'consultantpro' ),
			'success' => esc_html__( 'Success.', 'consultantpro' ),
			'install_callback' => '_content_install_settings',
		);
		$content['codestar'] = array(
			'title' => esc_html__( 'Theme options', 'consultantpro' ),
			'description' => esc_html__( 'Change logo colors, fonts, layout... you know, the cool stuff.','consultantpro' ),
			'pending' => esc_html__( 'Pending.','consultantpro' ),
			'installing' => esc_html__( 'Installing Default Theme Settings.','consultantpro' ),
			'success' => esc_html__( 'Success.','consultantpro' ),
			'install_callback' => '_content_install_codestar',
		);
		$content['customize'] = array(
			'title' => esc_html__( 'Customize options', 'consultantpro' ),
			'description' => esc_html__( "You'll see-more cool stuff to play with.", 'consultantpro' ),
			'pending' => esc_html__( 'Pending.', 'consultantpro' ),
			'installing' => esc_html__( 'Installing Default Customize Settings.', 'consultantpro' ),
			'success' => esc_html__( 'Success.', 'consultantpro' ),
	...
	public function envato_setup_default_content() {
		?>
		<h1><?php esc_html_e( 'Default Content', 'consultantpro' ); ?></h1>
		<form method="post">
			<p><?php printf( esc_html__( "Most theme's come with some \"Lorem ipsum...\" type of text that sounds like a language from Mars. consultant is different (but you know that by now!). We've handpicked some interesting articles that you can use or discard. Read them--they'll help you improve your blog. And have fun playing with the different options below. ", 'consultantpro' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
			<table class="envato-setup-pages" cellspacing="0">
				<thead>
				<tr>
					<td class="check"> </td>
					<th class="item"><?php esc_html_e( 'Item', 'consultantpro' ); ?></th>
					<th class="description"><?php esc_html_e( 'Description', 'consultantpro' ); ?></th>
					<th class="status"><?php esc_html_e( 'Status', 'consultantpro' ); ?></th>
				</tr>
				</thead>
				<tbody>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php esc_html_e( 'Continue', 'consultantpro' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'consultantpro' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		</form>
	...
	public function ajax_content(){
		...
		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])){
			wp_send_json_error(array('error'=>1,'message'=>esc_html__('No content Found','consultantpro')));
		}
		...
		if(! empty($json) ){
			...
			wp_send_json(array('error'=>1,'message'=>esc_html__('Error','consultantpro')));
		}
	...
	private function _import_wordpress_xml_file($xml_file_path){
		...
		if ( ! class_exists( 'WP_Import' )) {
			...
			if ( file_exists( $class_wp_importer ) )
				require_once get_template_directory() . '/inc/importer/importer/wordpress-importer.php';
		}

		if ( class_exists( 'WP_Import' ) )
		{
			require_once get_template_directory() . '/inc/importer/importer/envato-content-import.php';
	...
	private function _content_install_formidable(){
		if ( class_exists('FrmXMLHelper') ) {
			FrmXMLHelper::import_xml(__DIR__ ."/content/formidable.xml");
			return true;
		}
		return false;
	}
	...
	public function envato_setup_logo_design() {
		...
		?>
		<h1><?php esc_html_e( 'Logo &amp; Design', 'consultantpro' ); ?></h1>
		<form method="post">
			<p><?php esc_html_e('Please add your logo below. For best results, the logo should be a transparent PNG ( 190 by 35 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.', 'consultantpro' ); ?> </p>
			<table>
				<tr>
					<td>
						<div id="current-logo">
					...
					<td>
						<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'consultantpro' ); ?></a>
					</td>
				</tr>
			</table>


			<p><?php esc_html_e('Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'consultantpro' ); ?></p>
			...
			<p class="envato-setup-actions step">
				<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'consultantpro' ); ?>" name="save_step" />
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'consultantpro' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		</form>
	...
	public function envato_setup_customize(){
		...
		<h1><?php esc_html_e( 'Theme Customization', 'consultantpro' ); ?></h1>
		<p>
			<?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. You can "drag & drop" the widgets into your sidebar. To control which "widget areas" appear, go to an individual page and look for the "Left/Right Column" menu. Here widgets can be chosen for display on the left or right of a page. More details in documentation.', 'consultantpro' ); ?>
		</p>
		<p>
			<em>
			<?php echo sprintf(esc_html__( 'Advanced Users: If you are going to make changes to the theme source code please use a %s rather than modifying the main theme HTML/CSS/PHP code.', 'consultantpro' ), '<a href="https://codex.wordpress.org/Child_Themes" target="_blank">Child Theme</a>'); ?>
			<?php echo sprintf(esc_html__( 'This allows the parent theme to receive updates without overwriting your source code changes.%s See %s in the main folder for a sample.', 'consultantpro' ), '<br/>', '<code>child-theme.zip</code>'); ?>
			</em>
		</p>

		<p class="envato-setup-actions step">
			<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'consultantpro' ); ?></a>
		</p>
	...
	public function envato_setup_ready() {
		...
		<h1><?php esc_html_e( 'Your Website is Ready!', 'consultantpro' ); ?></h1>

		<p><?php esc_html_e( 'In the future, if you would like to modify your theme, simply login to your WordPress dashboard, and away you go!', 'consultantpro' ); ?></p>

		<div class="envato-setup-next-steps">
			<div class="envato-setup-next-steps-first">
				<h2><?php esc_html_e( 'Next Steps', 'consultantpro' ); ?></h2>
				<ul>
					<li class="setup-product"><a class="button button-primary button-large" target="_blank" href="<?php echo home_url('/'); ?>"><b><?php esc_html_e( 'VIEW YOUR NEW WEBSITE!', 'consultantpro' ); ?></b></a></li>
				</ul>
			</div>
			<div class="envato-setup-next-steps-last">
				<h2><?php esc_html_e( 'More Resources', 'consultantpro' ); ?></h2>
				<ul>
					<li class="documentation"><a href="<?php echo esc_url('#'); ?>" target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'consultantpro' ); ?></a></li>
					<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'consultantpro' ); ?></a></li>
				</ul>
			</div>
		</div>
		<?php
